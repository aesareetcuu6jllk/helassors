...
# دالة لتنسيق الشاشة (كبيرة وواضحة)
def big_screen(text):
    return f"```\n┌──────────────────┐\n  {text}  \n└──────────────────┘\n```"
...
async def handle_calc_press(event):
    ...
    if action == "AC":
        ...
        # تصحيح الخطأ هنا (استخدام current_val)
        if "=" in current_val or current_val == "Error" or current_val == "0":
        ...
        if "=" in current_val or current_val == "Error":
            ...
            if current_val == "0":
                if action in "+-×÷^%": return await event.answer("ابدأ برقم")
                new_display = action
                new_val = action
            else:
...
def create_pro_buttons():
    # تم إضافة مسافات للأزرار لتكبير حجمها
    return [
